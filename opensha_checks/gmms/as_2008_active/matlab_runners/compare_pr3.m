...
file_opensha = fullfile(dir_tests, 'outputs_opensha', 'pr3_opensha_expected.csv');
file_matlab = fullfile(dir_tests, 'outputs_matlab', 'pr3_f5_f10_after', 'pr3_results.csv');
...
plot(df_SA.T_equiv, df_SA.median_matlab, 'rx--', 'LineWidth', 1.5, ...
    'DisplayName', 'MATLAB (after PR3 fix)');

xlabel('Period T (s)');
ylabel('Median SA (g)');
title('PR3 Comparison: SA Medians (OpenSHA vs MATLAB)');
