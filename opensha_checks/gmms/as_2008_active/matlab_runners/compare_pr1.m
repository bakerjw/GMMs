...
file_opensha = fullfile(dir_tests, 'outputs_opensha', 'pr1_opensha_expected.csv');
file_matlab = fullfile(dir_tests, 'outputs_matlab', 'pr1_f4_after', 'pr1_results.csv');
...
plot(df_SA.T_equiv, df_SA.median_matlab, 'rx--', 'LineWidth', 1.5, ...
    'DisplayName', 'MATLAB (after PR1 fix)');

xlabel('Period T (s)');
ylabel('Median SA (g)');
title('PR1 Comparison: SA Medians (OpenSHA vs MATLAB)');
