function [median, sigma, period1] = as_2008_active(T,rup,site)
...
% Silva Ground-Motion Relations."Earthquake Spectra, 24(1), 67-97. 
%
...
%       delta           = dip angle of fault in degree
...
%                       = 0 otherwise
%       lambda          = rake angle, used to set reverse and normal
%                           faulting flags (FRV = 1 for reverse/reverse oblique for angles
%                           between 30 and 150 degrees, FNM = 1 for normal faulting angles
...
%       FVS30           = Vs30 flag: 1 for estimated Vs30, 0 for measured
...
% Set reverse and normal faulting flags based on rake angle
if rup.lambda > 30 && rup.lambda < 150
    FRV = 1;
else
    FRV = 0;
end
if  rup.lambda < -60 && rup.lambda > -120
    FNM = 1;
else 
    FNM = 0;
end

Td = 10^( -1.25 + 0.3*rup.M );
[SaTd] = AS_2008_nga_sub(rup.M,1100,Td,rup.Rrup,rup.Rjb,rup.Rx,rup.delta,rup.Ztor,0,rup.W,FRV,FNM,rup.AS,rup.HW,site.fvs30, 0, 0,1);

[median, sigma, period1]=AS_2008_nga_sub(rup.M,site.Vs30,T,rup.Rrup,rup.Rjb,rup.Rx,rup.delta,rup.Ztor,site.Z10,rup.W,FRV,FNM,rup.AS,rup.HW,site.fvs30,Td,SaTd,0);


function [median, sigma, period1] = AS_2008_nga_sub(M, Vs30, T, Rrup, Rjb, Rx, dip, Ztor, Z10, W, FRV, FNM, FAS, FHW, FVS30, Td, SaTd,irock)
% 
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% INPUT
...
[sigma, sigma, tau, pga_sigmaB, pga_tauB] = abrahamson_silva_sigma(M, pga_rock, Vs30,0, 0, get_abrahamson_silva_constants(1,Vs30,FVS30));
    ...
    if(T==1000)
        ...
        for index=3:1:nperi
            ...
            if period(index)<=Td || SaTd==0
                median(index-2) = exp(calc_val(M, Rrup, Rjb, Rx, dip, Ztor, Z10, W, FRV, FNM, FAS, FHW, pga_rock, Vs30, V) + (1-irock)* f_10(Z10, Vs30, V));
            else
                median(index-2) = exp(calc_val2(SaTd,Td, period(index), Z10, pga_rock, Vs30, V));
            end
            [sigma(index-2), sigma(index-2), tau, sigmaB, tauB] = abrahamson_silva_sigma(M, pga_rock, Vs30, pga_sigmaB, pga_tauB, V);
        end
    end
end
...
if(iflg==0)
    ...
    % interpolate between periods if neccesary  
        if (length(find(period == Teach)) == 0)
            ...
            V = get_abrahamson_silva_constants(index,Vs30,FVS30);
            if period(index)<=Td || SaTd==0
                median(it) = exp(calc_val(M, Rrup, Rjb, Rx, dip, Ztor, Z10, W, FRV, FNM, FAS, FHW, pga_rock, Vs30, V) + (1-irock)* f_10(Z10, Vs30, V));
            else
                median(it) = exp(calc_val2(SaTd,Td, period(index), Z10, pga_rock, Vs30, V));
            end

            [sigma(it), sigma(it), tau, sigmaB, tauB] = abrahamson_silva_sigma(M, pga_rock, Vs30, pga_sigmaB, pga_tauB, V);
